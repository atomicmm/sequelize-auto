...
pluralize.addSingularRule(/cms$/i,'cms')
const { upperFirst, camelCase, flow: compose } = require('lodash')
...
AutoSequelize.prototype.run = function(callback) {
    ...
    function generateText(err) {
        ...
        async.each(_.keys(self.tables), function(table, _callback) {
            ...
            const tableName = compose(camelCase,pluralize.singular, upperFirst)(table)
